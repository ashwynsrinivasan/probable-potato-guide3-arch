...
class SOAGUI(tk.Tk):
    def __init__(self):
        ...
        # --- Common Inputs ---
        common_frame = ttk.LabelFrame(control_panel, text="Common Parameters", padding="10")
        ...
        ttk.Label(common_frame, text="Wavelength(s) (nm) [1290-1330]:").pack(pady=(5, 2), anchor='w')
        ...
        # Generate Button
        ttk.Button(control_panel, text="Generate Plots vs. Length", command=self.generate_plots).pack(pady=10, fill=tk.X, padx=5)
        ttk.Button(control_panel, text="Plot vs. Wavelength", command=self.plot_performance_vs_wavelength).pack(pady=10, fill=tk.X, padx=5)
    ...
    def validate_inputs(self):
        ...
        return temp, lambda_nm_list, pout_nominal, pout_3s, j_nominal, j_3sigma, l_active

    def generate_plots(self):
        validated_data = self.validate_inputs()
        if validated_data is None: return
        
        T_C, lambda_nm_list, target_Pout_dBm_nominal, target_Pout_dBm_3s, j_nominal, j_3sigma, l_active_annotate = validated_data
        ...
        fig.suptitle(f'SOA Performance vs. Active Length (T={T_C:.0f}°C, λ={lambda_to_plot:.0f}nm)', fontsize=16)
        ...
        if self.nominal_var.get():
            plot_data_nominal = self.calculate_plot_data(target_Pout_dBm_nominal, T_C, lambda_to_plot)
        ...
        if self.sigma_var.get():
            plot_data_3s = self.calculate_plot_data(target_Pout_dBm_3s, T_C, lambda_to_plot)
    ...
    def plot_performance_vs_wavelength(self):
        ...
        T_C, lambda_nm_list, target_Pout_dBm_nominal, target_Pout_dBm_3s, j_nominal, j_3sigma, l_active = validated_data
        ...
        fig.suptitle(f'SOA Performance vs. Wavelength\nL_active={l_active:.0f}µm, T={T_C:.0f}°C', fontsize=16)

        if self.nominal_var.get():
            plot_data = self.calculate_vs_wavelength_data(l_active, T_C, j_nominal, target_Pout_dBm_nominal, lambda_sweep_nm)
        ...
        if self.sigma_var.get():
            plot_data_3s = self.calculate_vs_wavelength_data(l_active, T_C, j_3sigma, target_Pout_dBm_3s, lambda_sweep_nm)
        ...
        axes[1].set_ylabel('Saturated Gain (dB)'); axes[1].grid(True); axes[1].legend()
        axes[2].set_ylabel('Required P_in (dBm)'); axes[2].set_xlabel('Wavelength (nm)'); axes[2].grid(True); axes[2].legend()

        plt.tight_layout(rect=[0, 0, 1, 0.96])
        self.embed_plot(fig)

    def calculate_vs_wavelength_data(self, l_active, T_C, J_val, target_Pout_dBm, lambda_sweep):
        target_Pout_mW = 10**(target_Pout_dBm / 10.0)
        results = {'lambda': [], 'wpe': [], 'gain': [], 'pin': []}
        
        soa_inst = SOA(L_active_um=l_active, verbose=False)
