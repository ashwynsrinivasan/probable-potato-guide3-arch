...
class SOA:
    ...
    def find_Pin_for_target_Pout(self, target_Pout_mW: float, I_mA: float, lambda_nm: float, T_C: float) -> Union[float, None]:
        """
        Find required input power for target output power.
        
        Args:
            target_Pout_mW (float): Target output power in mW
            I_mA (float): Current in mA
            lambda_nm (float): Wavelength in nm
            T_C (float): Temperature in Celsius
            
        Returns:
            Union[float, None]: Required input power in mW, or None if not achievable
        """
        J_kA_cm2 = self.calculate_current_density_kA_cm2(I_mA)
        if (J_kA_cm2 <= 1e-9 and I_mA > 1e-9) or (I_mA <= 1e-9 and target_Pout_mW > 1e-9):
            return None
        
        def objective_func(Pin_mW_local: float) -> float:
            if Pin_mW_local <= 0:
                return target_Pout_mW * 100 + 1
